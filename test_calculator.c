...
void tearDown(void) {
    ...
    // For example, free dynamically allocated memory, reset global variables, etc.
    // In this example, we do not need anything here.
}
...
void test_subtract_overflow(void) {
    int result = subtract(INT_MIN, 1); // Should wrap around or cause undefined behavior
    TEST_ASSERT_TRUE(result > 0); // This checks if overflow occurred
}
...
void test_multiply_negative_numbers(void) {
    TEST_ASSERT_EQUAL(12, multiply(-3, -4)); // We expect -3*-4 to be 12
}
